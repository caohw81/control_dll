// e_control_dll.cpp : Defines the initialization routines for the DLL.


#include "stdafx.h"
#include "e_control_dll.h"
...
BEGIN_MESSAGE_MAP(CE_control_dllApp, CWinApp)
	//{{AFX_MSG_MAP(CE_control_dllApp)
...
/////////////////////////////////////////////////////////////////////////////
// CE_control_dllApp construction

CE_control_dllApp::CE_control_dllApp()
{
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CE_control_dllApp object

CE_control_dllApp theApp;
...
int udpSocketInit(){
	...
	CString m_send_ip;		
	GetPrivateProfileString("e_connect","send_ip","172.18.18.11",m_send_ip.GetBuffer(MAX_PATH),MAX_PATH,strIniPath);

	CString send_Port="51888";
	GetPrivateProfileString("e_connect","send_Port","51888",send_Port.GetBuffer(MAX_PATH),MAX_PATH,strIniPath);
	...
	for(int i=0;i<m_ip_recvNum;++i){
		...
		GetPrivateProfileString("e_connect",ip_addr_str,ip_addr,ip_addr.GetBuffer(MAX_PATH),MAX_PATH,strIniPath);
		m_recvIpVec.push_back(ip_addr.GetBuffer(0));
		
		CString ip_Port;
		ip_Port.Format("recv_Port%d",i+1);		
		GetPrivateProfileString("e_connect",ip_Port,recv_Port,recv_Port.GetBuffer(MAX_PATH),MAX_PATH,strIniPath);	
...
void dispRecvedMsg(int ipIndex, char* p_buff, int len){
	...
	*/
    ...
    if (pRootElement){
		...
		Log("4g_msg_recv ok @@ ipIndex:%d, msg:%s",ipIndex,secLeaf_str.c_str());
		if(secLeaf_str=="status_response"){			//%%心跳 
			...
			if(status_char){
				...
				//AfxMessageBox("0");
				Log("4g_status_response ok @@ ipIndex:%d, m_status:%d",ipIndex,m_status);
...
*/			
			...
			Log("4g_femto_status ok @@ ipIndex:%d,status:%d,euarfcn:%d,mcc:%d,mnc:%s,pci:%d,periodTac:%d,txpower:%d,maxTxPower:%d,tac:%d",ipIndex,m_status, m_euarfcn,m_mcc,m_mnc_char,m_pci,m_periodTac,m_txpower,m_maxTxPower,m_tac);
			...
			TiXmlElement* p_rebootflag= p_secLeaf->FirstChildElement("rebootflag");
			const char *rebootflag_char = p_rebootflag->GetText(); 		
			
			if(result_char){
				int m_result=atoi(result_char);
				int m_rebootflag=atoi(rebootflag_char);
				//printf("set_configuration_result status:%d\n",m_status);
				//CString str;
				//str.Format("config status:%d",m_status);
				//AfxMessageBox(str);
				Log("4g_set_configuration_result ok @@ ipIndex:%d,m_result:%d,m_rebootflag:%d",ipIndex, m_result,m_rebootflag);
			...
			Log("4g_set_redirection_rsp ok @@ ipIndex:%d,m_result:%d,m_rebootflag:%d",ipIndex,m_result,m_rebootflag);
			...
			Log("4g_activate_nodeb_result ok @@ ipIndex:%d, m_result:%d,m_rebootflag:%d",ipIndex, m_result,m_rebootflag);
			...
			Log("4g_imsi_list_config_result ok @@ ipIndex:%d,m_result:%d",ipIndex, m_result);
			...
			if(p_whiteimsi){
				...
				if(p_w_imsi){
					while(p_w_imsi){
						...
						Log("4g_white_imsi_list_check_result @@ ipIndex:%d, whiteimsi:%s",ipIndex, imsi.c_str());
						whiteVec.push_back(imsi);
						p_w_imsi=p_w_imsi->NextSiblingElement();
					}
					int imsiNum=whiteVec.size();
					if(imsiNum>0){
						char* whiteImsiList=(char* )malloc(imsiNum*15*sizeof(char)+1);
						...
						whiteImsiList[imsiNum*15*sizeof(char)]='\0';
						Log("4g_white_imsi_list_check_result ok @@ ipIndex:%d, white imsi Num:%d",ipIndex, imsiNum);
						(*pWhiteImsiCheckFun)(ipIndex, whiteImsiList,imsiNum );
						free(whiteImsiList);
					}					
				}else{
					Log("4g_white_imsi_list_check_result ok @@ ipIndex:%d, white imsiNum:%d",ipIndex, 0);
			...
			if(p_blackimsi){
				...
				if(p_b_imsi){
					while(p_b_imsi){
						...
						Log("4g_black_imsi_list_check_result @@ ipIndex:%d, blackimsi:%s",ipIndex, imsi.c_str());
					...
					if(imsiNum>0){					
						char* blackImsiList=(char* )malloc(imsiNum*15*sizeof(char)+1);
						memset(blackImsiList,0,imsiNum*15);
						for(int i=0;i<imsiNum;++i){
							memcpy(&blackImsiList[i*15],blackVec[i].c_str(),15);
						}
						Log("4g_black_imsi_list_check_result ok @@ ipIndex:%d, black imsi Num:%d",ipIndex, imsiNum);
						blackImsiList[imsiNum*15*sizeof(char)]='\0';
						(*pBlackImsiCheckFun)(ipIndex, blackImsiList,imsiNum );						
						free(blackImsiList);
					}				
				}else{
					Log("4g_black_imsi_list_check_result ok @@ ipIndex:%d, black imsiNum:%d",ipIndex, 0);
			...
			Log("4g_imsi_list_delconfig_result ok @@ ipIndex:%d, m_result:%d",ipIndex, m_result);
			...
			Log("4g_scanner ok @@ ipIndex:%d, userType:%d,imsi:%s,imei:%s,rsrp:%d",ipIndex, m_userType,imsiInfo.imsi,imsiInfo.imei,m_rsrp);
...
DllExport void e_femtoStatusRequest(int ipIndex){
	TiXmlDocument *docXml = new TiXmlDocument();
	TiXmlDeclaration* decl = new TiXmlDeclaration("1.0", "UTF-8", "");
	docXml->LinkEndChild(decl);
	...
	if (udpConnectFlag){
		if(sendMsg(ipIndex,buff,lenth-1)<=0){
			Log("4g_femto_status_request error && ipIndex:%d",ipIndex);
		}
...
DllExport void e_setCellConfig(int ipIndex, int euarfcn, int mcc,char* mnc, int pci, int tac, int txpower, int periodTac, int bandwidth, char* cellid){
	TiXmlDocument *docXml = new TiXmlDocument();
	TiXmlDeclaration* decl = new TiXmlDeclaration("1.0", "UTF-8", "");
	docXml->LinkEndChild(decl);
...
*/
	...
	if (udpConnectFlag){
		Log("4g_setCellConfig begin && ipIndex:%d, euarfcn:%d,mcc:%d,mnc:%s,pci:%d,tac:%d,periodTac:%d",ipIndex, euarfcn,mcc,mnc,pci,tac,periodTac);
		if(sendMsg(ipIndex,buff,lenth-1)<=0){
			Log("4g_setCellConfig error && ipIndex:%d, euarfcn:%d,mcc:%d,mnc:%s,pci:%d,tac:%d,periodTac:%d",ipIndex, euarfcn,mcc,mnc,pci,tac,periodTac);
		}
...
/*DllExport void e_setRedirection(int ipIndex, int category, char* priority,int GeranRedirect, int arfcn, int UtranRedirect, int uarfcn, int EutranRedirect, int earfcn,int RejectMethod){
	TiXmlDocument *docXml = new TiXmlDocument();
	TiXmlDeclaration* decl = new TiXmlDeclaration("1.0", "UTF-8", "");
	docXml->LinkEndChild(decl);
...
//==============================
	...
	buff[lenth]='\0';
#endif
	Log("4g_setRedirection@@ipIndex:%d,%s",buff);
	Log("4g_setRedirection@@ipIndex:%d, category:%d,priority:%s,GeranRedirect:%d,arfcn:%d,UtranRedirect:%d,uarfcn:%d,EutranRedirect:%d,earfcn:%d,RejectMethod:%d",ipIndex, category, priority,GeranRedirect, arfcn, UtranRedirect, uarfcn, EutranRedirect, earfcn,RejectMethod);
	if (udpConnectFlag){
...
//==============================
	...
	if (udpConnectFlag){
		...
		if(sendMsg(ipIndex, buff,lenth-1)<=0){
			Log("4g_setBlackRedirection error && ipIndex:%d, arfcn:%d",ipIndex, arfcn);
		}else{
			Log("4g_setBlackRedirection end && ipIndex:%d, arfcn:%d",ipIndex, arfcn);
		}
	}
	delete docXml;
	docXml = NULL;
...
DllExport void e_setWhiteRedirection(int ipIndex,int arfcn){
	TiXmlDocument *docXml = new TiXmlDocument();
	TiXmlDeclaration* decl = new TiXmlDeclaration("1.0", "UTF-8", "");
	docXml->LinkEndChild(decl);
...
//==============================
	...
	if (udpConnectFlag){
		if(sendMsg(ipIndex, buff,lenth-1)<=0){
			Log("4g_setWhiteRedirection error && ipIndex:%d, arfcn:%d",ipIndex, arfcn);
		}
		else{
			Log("4g_setWhiteRedirection end && ipIndex:%d, arfcn:%d",ipIndex, arfcn);
		}
	}
...
DllExport void e_activeCell(int ipIndex,int active_mode, int mode){
	TiXmlDocument *docXml = new TiXmlDocument();
	TiXmlDeclaration* decl = new TiXmlDeclaration("1.0", "UTF-8", "");
	docXml->LinkEndChild(decl);
...
*/	
	...
	if (udpConnectFlag){
		if(sendMsg(ipIndex, buff,lenth-1)<=0){
			Log("4g_activeCell error @@ ipIndex:%d, active_mode:%d,mode:%d",ipIndex, active_mode,mode);
		}else{
...
DllExport void e_addWhiteImsi(int ipIndex, char* whiteImsi){
	TiXmlDocument *docXml = new TiXmlDocument();
	TiXmlDeclaration* decl = new TiXmlDeclaration("1.0", "UTF-8", "");
	docXml->LinkEndChild(decl);
...
*/	
	...
	if (udpConnectFlag){
		if(sendMsg(ipIndex, buff,lenth-1)<=0){
			Log("4g_addWhiteImsi error && ipIndex:%d, whiteImsi:%s",ipIndex, whiteImsi);
		}else{
...
DllExport void e_addBlackImsi(int ipIndex,char* blackImsi){
	TiXmlDocument *docXml = new TiXmlDocument();
	TiXmlDeclaration* decl = new TiXmlDeclaration("1.0", "UTF-8", "");
	docXml->LinkEndChild(decl);
...
*/	
	...
	if (udpConnectFlag){
		if(sendMsg(ipIndex, buff,lenth-1)<=0){
			Log("4g_addBlackImsi error && ipIndex:%d, blackImsi:%s",ipIndex, blackImsi);
		}esle{
...
DllExport void e_deleteWhiteImsi(int ipIndex,char* whiteImsi, int n){
	char m_imsi[16];
	TiXmlDocument *docXml = new TiXmlDocument();
	TiXmlDeclaration* decl = new TiXmlDeclaration("1.0", "UTF-8", "");
	docXml->LinkEndChild(decl);
...
*/	
	...
	if (udpConnectFlag){
		if(sendMsg(ipIndex, buff,lenth-1)<=0){
			Log("4g_deleteWhiteImsi error && ipIndex:%d, num:%d,whiteImsi:%s",ipIndex, n,whiteImsi);
		}else{
...
DllExport void e_deleteBlackImsi(int ipIndex,char* blackImsi,int n){
	char m_imsi[16];
	TiXmlDocument *docXml = new TiXmlDocument();
	TiXmlDeclaration* decl = new TiXmlDeclaration("1.0", "UTF-8", "");
	docXml->LinkEndChild(decl);
...
*/	
	...
	if (udpConnectFlag){
		if(sendMsg(ipIndex, buff,lenth-1)<=0){
			Log("4g_deleteBlackImsi error && ipIndex:%d, num:%d,blackImsi:%s",ipIndex, n,blackImsi);
		}else{
...
DllExport void e_imsiListCheck(int ipIndex){
	TiXmlDocument *docXml = new TiXmlDocument();
	TiXmlDeclaration* decl = new TiXmlDeclaration("1.0", "UTF-8", "");
	docXml->LinkEndChild(decl);
...
*/	
	...
	if (udpConnectFlag){
		if(sendMsg(ipIndex,buff,lenth-1)<=0){
			Log("4g_imsiListCheck error && ipIndex:%d",ipIndex);
		}else{
...
DllExport void e_quit(int ipIndex){
	udpConnectFlag=0;
	if(mserver)
		closesocket(mserver);	
...
DllExport int e_getIMSIReportCallBack(int (*pFun)(E_IMSIReportInfo*p)){ //IMSI上报
	pIMSIReportFun = pFun;
	return 1;
}

DllExport int e_getStatusCallBack(int (*pFun)(int ipIndex, int id,int param)){ //状态 定位值
	pStatusFun = pFun;
	return 1;
}

DllExport int e_whiteImsiCheckCallBack(int (*pFun)(int ipIndex, char*imsiList, int rows)){ //状态 定位值
	pWhiteImsiCheckFun = pFun;
	return 1;
}

DllExport int e_blackImsiCheckCallBack(int (*pFun)(int ipIndex, char*imsiList, int rows)){ //状态 定位值
	pBlackImsiCheckFun = pFun;
	return 1;
}

DllExport int e_cellStatusCallBack(int (*pFun)(int ipIndex, int status, int earfcn,int mcc,char* mnc, int pci, int periodTac,int txpower)){ //状态 定位值
