...
void CE_control_dll_testDlg::OnInitRun() {
	...
	hDll = LoadLibrary("e_control_dll.dll");
	...
	e_statusReportCallBack pf1=(e_statusReportCallBack)GetProcAddress(hDll,"e_getStatusCallBack");
	if(pf1)
	{
	...
	e_getIMSIReportCallBack pf2=(e_getIMSIReportCallBack)GetProcAddress(hDll,"e_getIMSIReportCallBack");
	if(pf2)
	{
	...
	e_whiteIMSICheckCallBack pf3=(e_whiteIMSICheckCallBack)GetProcAddress(hDll,"e_whiteImsiCheckCallBack");
	if(pf3)
	{
	...
	e_blackIMSICheckCallBack pf4=(e_blackIMSICheckCallBack)GetProcAddress(hDll,"e_blackImsiCheckCallBack");
	if(pf4)
	{
	...
	e_cellStatusCallBack pf5=(e_cellStatusCallBack)GetProcAddress(hDll,"e_cellStatusCallBack");
	if(pf5)
	{
		pf5(e_cellStatusFun);
	}
	...
	e_initRun pInit = (e_initRun)GetProcAddress(hDll,"e_initRun");
...
/*pow_level   1-5   max:5*/
	CELLCONFIG pCellConfig = (CELLCONFIG)GetProcAddress(hDll,"e_setCellConfig");
...
/*pow_level   1-5   max:5*/
	REDIRECTION pRedirect = (REDIRECTION)GetProcAddress(hDll,"e_setRedirection");
...
/*pow_level   1-5   max:5*/
	ACTIVECELL pActiveCell = (ACTIVECELL)GetProcAddress(hDll,"e_activeCell");
...
/*pow_level   1-5   max:5*/
	ADDWHITEIMSI pAddWhiteImsi = (ADDWHITEIMSI)GetProcAddress(hDll,"e_addWhiteImsi");
...
/*pow_level   1-5   max:5*/
	ADDWHITEIMSI pAddBlackImsi = (ADDWHITEIMSI)GetProcAddress(hDll,"e_addBlackImsi");
...
/*pow_level   1-5   max:5*/
	DELETEWHITEIMSI pDeleteWhiteImsi = (DELETEWHITEIMSI)GetProcAddress(hDll,"e_deleteWhiteImsi");
...
/*pow_level   1-5   max:5*/
	DELETEBLACKIMSI pDeleteBlackImsi = (DELETEBLACKIMSI)GetProcAddress(hDll,"e_deleteBlackImsi");
...
/*pow_level   1-5   max:5*/
	IMSILISTCHECK pImsiListCheck = (IMSILISTCHECK)GetProcAddress(hDll,"e_imsiListCheck");
...
void CE_control_dll_testDlg::cellStatusCheck() 
{
	...
	/*pow_level   1-5   max:5*/
	FEMTOSTATUSCHECK pFemtoStatusCheck = (FEMTOSTATUSCHECK)GetProcAddress(hDll,"e_femtoStatusRequest");
